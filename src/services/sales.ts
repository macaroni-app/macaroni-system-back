...
export const salesService = {
  getAll: (options: FilterQuery<SaleType>) => {
    try {
      return Sale.find({ ...options }).populate('client').populate('business').populate('paymentMethod').populate('orderRequest').populate('createdBy', ['firstName', 'lastName']).sort({ sortingDate: -1 })
    } catch (error) {
      return error
    }
  },
